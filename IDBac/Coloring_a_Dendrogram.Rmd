...
In this case I took a matrix (where rows = samples, and columns = binned peaks) and called `dist()` on it to compute a distance matrix. Then I computed a dendrogram with `hclust()`
In IDBac it's a bit more complicated than this (had to create own cosine function, interactivity, each function has settings, etc ) but this is the easiest for explaining.
